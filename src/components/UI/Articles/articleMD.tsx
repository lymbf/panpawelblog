    ...
    return (
        <div className='flex flex-row mb-6'>
            <div className={'flex flex-col'}>
                <Image className={'rounded-2xl'} src={`${process.env.BASE_URL}${article.image_link}`} alt={article.title || ''} width={190}
                       height={126}/>
                <div className={'w-full flex flex-row justify-center'}>{article.tags && article.tags.map((t,i)=>{
                    return <div className={'mt-1.5 tracking-wider text-slate-500 text-[10px] capitalize flex flex-row'}>{t.name}
                        {article.tags && i!=article.tags.length-1 && <div className = 'ml-2 mr-2  flex items-center justify-center'>|</div>}</div>
                })}</div>
            </div>
            <div className={'w-[198px] ml-5 flex flex-col justify-center pb-5'}>
                ...
                <h2 className={'text-base leading-5 mb-2'}>
                    {article.title}
                </h2>
